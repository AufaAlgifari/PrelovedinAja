...
function ensureMockProductsInDatabase() {
    ...
    try {
        if (User::where('email', $firstMockEmail)->exists()) {
            return;
        }
    } catch (\Exception $e) {
        return; // DB not ready or migrated yet
    }
    ...
    $mocks = [
        ...
        [
            ...
            'seller' => [
                'name' => 'Rian - Faperta',
                'email' => 'rian.faperta@unsoed.ac.id',
        ...
        [
            ...
            'seller' => [
                ...
                'email' => 'bagas.fisip@unsoed.ac.id',
    ...
    foreach ($mocks as $mock) {
        $sellerData = $mock['seller'];
        $user = User::updateOrCreate(
            ['email' => $sellerData['email']],
            [
                'name' => $sellerData['name'],
                'password' => bcrypt('password123'),
                'phone_number' => '0812-0000-' . rand(1000, 9999),
                'unsoed_faculty' => $sellerData['unsoed_faculty'],
                'unsoed_major' => $sellerData['unsoed_major'],
                'avatar_url' => $sellerData['avatar_url'],
                'rating_cache' => $sellerData['rating_cache'],
                'role' => 'student',
                'is_verified' => true,
                'no_kampus' => 'H1D0' . rand(20000, 29999)
            ]
        );

        $categoryObj = Category::where('name', $mock['category'])->first();
...
Route::get('/products/{id}', function ($id) {
    ...
    if (!$product && (int)$id <= 10) {
        $mocks = getMockProducts();
        $mock = collect($mocks)->firstWhere('id', (int)$id);
        if ($mock) {
            try {
                $dbProduct = Product::with('seller')->where('title', $mock['title'])->first();
                if ($dbProduct) {
                    $product = $dbProduct;
                    $isDb = true;
                }
