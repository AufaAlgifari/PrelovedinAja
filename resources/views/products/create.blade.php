...
@section('content')
<div class="min-h-[calc(100vh-80px)] py-12 px-4 sm:px-6 lg:px-8 bg-[#FBF6EC]">
    <div class="max-w-3xl mx-auto bg-[#F5E4B0] rounded-3xl border border-[#D4A017]/25 shadow-xl p-6 sm:p-10 relative overflow-hidden">
        
        <!-- Background graphics -->
        <div class="absolute -top-24 -left-24 w-48 h-48 bg-[#D4A017]/5 rounded-full blur-3xl"></div>
        <div class="absolute -bottom-24 -right-24 w-48 h-48 bg-[#7A4A10]/5 rounded-full blur-3xl"></div>

        <div class="border-b border-[#D4A017]/20 pb-5 mb-8 relative">
            <h1 class="text-3xl font-extrabold text-[#2E1A06] tracking-tight font-heading">Jual Barang Preloved</h1>
            <p class="text-xs text-[#7A4A10] mt-1 font-medium">Isi detail produk Anda untuk dipublikasikan ke seluruh mahasiswa UNSOED.</p>
        </div>
        ...
        <form id="upload-form" class="space-y-6 relative" onsubmit="handleUpload(event)">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-[#7A4A10] uppercase tracking-wider mb-2">Nama Barang / Produk</label>
                    <input type="text" id="title" required placeholder="Misal: Kalkulator Casio FX-991EX, Buku Kalkulus Purcell..." 
                           class="w-full px-4 py-3 bg-[#FBF6EC] border border-[#D4A017]/30 rounded-2xl text-sm focus:border-[#7A4A10] focus:ring-4 focus:ring-[#7A4A10]/10 focus:outline-none transition-all text-[#2E1A06]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-[#7A4A10] uppercase tracking-wider mb-2">Harga Jual (Rp)</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-[#7A4A10] text-sm font-bold">Rp</span>
                        <input type="number" id="price" required min="1000" placeholder="50000" 
                               class="w-full pl-10 pr-4 py-3 bg-[#FBF6EC] border border-[#D4A017]/30 rounded-2xl text-sm focus:border-[#7A4A10] focus:ring-4 focus:ring-[#7A4A10]/10 focus:outline-none transition-all font-bold text-[#7A4A10]">
                    </div>
                </div>
            </div>
            ...
            <!-- Description -->
            <div>
                <label class="block text-xs font-bold text-[#7A4A10] uppercase tracking-wider mb-2">Deskripsi Produk Lengkap</label>
                <textarea id="description" required rows="4" placeholder="Jelaskan detail barang Anda. Misal: masa pakai, kelengkapan aksesoris, minus lecet fisik, alasan dijual, atau tempat COD ideal..." 
                          class="w-full px-4 py-3 bg-[#FBF6EC] border border-[#D4A017]/30 rounded-2xl text-sm focus:border-[#7A4A10] focus:ring-4 focus:ring-[#7A4A10]/10 focus:outline-none transition-all leading-relaxed text-[#2E1A06]"></textarea>
            </div>

            <!-- File Upload & Preview -->
            <div>
                <label class="block text-xs font-bold text-[#7A4A10] uppercase tracking-wider mb-2">Foto Barang</label>
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-[#D4A017]/30 border-dashed rounded-3xl hover:border-[#7A4A10] bg-[#FBF6EC]/50 hover:bg-[#FBF6EC] transition-all duration-200 relative group cursor-pointer" onclick="document.getElementById('file-upload').click()">
                    ...
                    <div class="space-y-2 text-center" id="upload-prompt">
                        <svg class="mx-auto h-12 w-12 text-[#7A4A10]/70 group-hover:text-[#7A4A10] transition-colors" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                            <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                        <div class="flex text-sm justify-center text-[#2E1A06] font-bold">
                            <span class="text-[#7A4A10] hover:text-[#2E1A06]">Unggah file foto</span>
                            <p class="pl-1">atau seret ke sini</p>
                        </div>
                        <p class="text-[10px] text-[#7A4A10]/60 font-bold uppercase">PNG, JPG, WEBP hingga 2MB</p>
                    </div>
                    <!-- Image Preview Element -->
                    <div id="preview-container" class="hidden absolute inset-0 bg-[#FBF6EC] rounded-3xl flex items-center justify-center p-2 border border-[#D4A017]/30 shadow-inner group">
                        <img id="image-preview" src="#" alt="Pratinjau Foto" class="max-h-full max-w-full object-contain rounded-2xl">
                        <button type="button" onclick="removePreview(event)" class="absolute top-4 right-4 bg-[#2E1A06]/70 hover:bg-rose-600 text-[#FBF6EC] rounded-full p-2.5 shadow transition-all scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100">
                            ✕ Hapus Foto
                        </button>
                    </div>
                </div>
...
<script>
    ...
    async function handleUpload(e) {
        ...
        // Create standard multipart FormData payload instead of raw Base64 string in JSON
        ...
        try {
            ...
            console.log('Database API offline, using fallback frontend local storage.');
        }
